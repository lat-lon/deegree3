...
If you downloaded all four example workspaces (as described in <<anchor-lightly>>), set an administration console password before you continue and apply proxy parameters, if needed.
Your _.deegree_ workspace directory will look like this:
...
with a random value. Alternatively, a value of `*` in _config.apikey_ will
