...
* <<anchor-deegree-webapps,deegree webservices webapp>> itself.
...
the administration console is described in <<anchor-global-configuration>>. The
...
WARNING: The administration console and REST API provide read and write access to the server's file system. Therefore, you must not operate the Java Servlet container as root user! Consider to use the <<anchor-deegree-webapps,read-only minimal webapp>> instead. Furthermore, you should consider to enable the Java Security Manager and define restrictive file permissions.footnote:[How to run securely Java
...
[[anchor-deegree-webapps]]
==== deegree webservices basic and minimal webapps

Since deegree webservices version 3.6 the project provides three different flavours of the webapp:

- `deegree-webservices.X.Y.war`: is the default (standard) webapp including the <<anchor-configuration-restapi,REST-API>> and <<anchor-admin-console,administration console>>.
- `deegree-webservices-basic-X.Y.war`: this webapp includes the <<anchor-configuration-restapi,REST-API>> but *not* the administration console!
...
TIP: For productive deployments it is recommended to use the read-only variant `deegree-webservices-minimal-X.Y.war`!